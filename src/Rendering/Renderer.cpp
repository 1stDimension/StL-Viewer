...
//    glBindVertexArray(this->vao);
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 sizeof(float) *  3 * 3 * contentSplitter->getNumNormals() // 3 normals 1 per vertex each is 3 floats
            , contentSplitter->getNormals(), GL_STATIC_DRAW );
    ...
    this->u_M_location = shaderHandler.getUniformLocation("u_M");
    this->u_V_location = shaderHandler.getUniformLocation("u_V");
    this->u_P_location = shaderHandler.getUniformLocation("u_P");
...
void Renderer::draw() {
    ...
    if (u_M_location != -1)
        glUniformMatrix4fv(u_M_location, 1, GL_FALSE, glm::value_ptr(this->model));
    if (u_V_location != -1)
        glUniformMatrix4fv(u_V_location, 1, GL_FALSE, glm::value_ptr(this->view));
    if (u_P_location != -1)
